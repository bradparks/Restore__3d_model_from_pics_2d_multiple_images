        ...
        namespace {
            ...
            WallTime WallTimeImp::Now() {
                ...
                do {
                    ...
                    if (top_bits
                        == last_adjust_time_) {  // don't need to recompute drift
            ...
            WallTimeImp::WallTimeImp()
                ...
                max_interval_cycles_ =
                    static_cast<int64_t>(cycles_per_second_
                        * kMaxErrorInterval);
                do {
                    base_cycletime_ = cycleclock::Now();
                    base_walltime_ = Slow();
                } while (cycleclock::Now() - base_cycletime_
                    > max_interval_cycles_);
                ...
                SetDrift(0.0);
                last_adjust_time_ =
                    static_cast<uint32_t>(uint64_t(base_cycletime_) >> 32);
            }
            ...
            WallTime ChronoWalltimeNow() {
                ...
                typedef std::chrono::duration<WallTime,
    ...
    namespace {
        ...
        std::string DateTimeString(bool local) {
            ...
            if (local) {
                ...
                written =
                    std::strftime(storage, sizeof(storage), "%F %T", &timeinfo);
#endif
            } else {
#if defined(BENCHMARK_OS_WINDOWS)
