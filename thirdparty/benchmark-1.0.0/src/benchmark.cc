...
// Timer management class
        class TimerManager {
          public:
            ...
            void StartTimer() EXCLUDES(lock_) {
                bool last_thread = false;
                {
                    ...
                    if (last_thread) {
                        CHECK(!running_)
                            << "Called StartTimer when timer is already running";
                        running_ = true;
            ...
            void StopTimer() EXCLUDES(lock_) {
                bool last_thread = false;
                {
                    ...
                    if (last_thread) {
                        CHECK(running_)
                            << "Called StopTimer when timer is already stopped";
                        InternalStop();
                    }
                }
                if (last_thread) {
          ...
          private:
            ...
            int
                entered_;         // Number of threads that have entered this barrier
...
// benchmark identifies a family of related benchmarks to run.
        ...
        class BenchmarkImp {
          public:
            ...
            static void
                AddRange(std::vector<int> *dst, int lo, int hi, int mult);
          ...
          private:
            ...
            std::vector<std::pair<int, int> >
                args_;  // Args for all benchmark runs
            double min_time_;
        ...
        bool BenchmarkFamilies::FindBenchmarks(
            ...
            if (!re.Init(spec, &error_msg)) {
                std::cerr << "Could not compile benchmark re: " << error_msg
                    << std::endl;
                return false;
            }
                ...
                for (auto const &args : family->args_) {
                    ...
                    for (int num_threads : *thread_counts) {
                        ...
                        if (!IsZero(family->min_time_)) {
                            instance.name += StringPrintF("/min_time:%0.3f",
                                                          family->min_time_);
                        }
                        ...
                        if (!family->thread_counts_.empty()) {
                            instance.name +=
                                StringPrintF("/threads:%d", instance.threads);
                        }
        ...
        void BenchmarkImp::AddRange(std::vector<int> *dst,
                                    int lo,
                                    int hi,
                                    int mult) {
            CHECK_GE(lo, 0);
    ...
    void RunBenchmark(const benchmark::internal::Benchmark::Instance &b,
        ...
        for (int i = 0; i < FLAGS_benchmark_repetitions; i++) {
            ...
            for (; ;) {
                ...
                Notification done;
                timer_manager =
                    std::unique_ptr<TimerManager>(new TimerManager(b.threads,
                                                                   &done));
                ...
                if (b.multithreaded) {
                    ...
                    for (std::size_t ti = 0; ti < pool.size(); ++ti) {
                        pool[ti] =
                            std::thread(&RunInThread, &b, iters, ti, &total);
                    }
                } else {
                ...
                const double
                    cpu_accumulated_time = timer_manager->cpu_time_used();
                const double
                    real_accumulated_time = timer_manager->real_time_used();
                timer_manager.reset();
                ...
                multiplier =
                    is_significant ? multiplier : std::min(10.0, multiplier);
