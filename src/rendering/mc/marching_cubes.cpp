...
void triangulate(const cube4c& values, const cubevec3f& points,
    ...
    vec3f vertlist[12];
    //
    // Determine the index in the edge table, which tells us
    ...
    for (lookup::lut_type s = 1; s <= sheets; s++) {
        ...
        switch (vertices_num) {
            case (7):
                tri.v3 = vertlist[idxptr[5]];
                tri.v2 = vertlist[idxptr[6]];
                triangles.push_back(tri);
            case (6):
                tri.v3 = vertlist[idxptr[4]];
                tri.v2 = vertlist[idxptr[5]];
                triangles.push_back(tri);
            case (5):
                tri.v3 = vertlist[idxptr[3]];
                tri.v2 = vertlist[idxptr[4]];
                triangles.push_back(tri);
            case (4):
                tri.v3 = vertlist[idxptr[2]];
                tri.v2 = vertlist[idxptr[3]];
                triangles.push_back(tri);
            case (3):
                tri.v3 = vertlist[idxptr[1]];
                tri.v2 = vertlist[idxptr[2]];
                triangles.push_back(tri);
        };
        //
        idxptr += vertices_num;
...
void perform_slices(const float* fgrid, const int_type dim_x,
    ...
    for (y = lbd_y; y < ubd_y; y += 2) {
        ...
        for (z = lbd_z; z < ubd_z; z += 2) {
            ...
            //
            // read first half cube as second. it will be copied
            // into the first half cube in the next for-loop.
            //
            values.hc1.c0 = grid[MC_COMPUTE_INDEX(dim_x, dim_xz, lbd_x, z, y)];
            values.hc1.c3 = grid[MC_COMPUTE_INDEX(dim_x, dim_xz, lbd_x, zz, y)];
            values.hc1.c6 =
                grid[MC_COMPUTE_INDEX(dim_x, dim_xz, lbd_x, zzz, y)];
            values.hc1.c1 = grid[MC_COMPUTE_INDEX(dim_x, dim_xz, lbd_x, z, yy)];
            values.hc1.c4 =
            ...
            // used for pre-check.
            //
            pre2 = (MC_ISOLEVEL_CHECK(values.hc1.data[0], isolevel) +
                    MC_ISOLEVEL_CHECK(values.hc1.data[1], isolevel) +
                    MC_ISOLEVEL_CHECK(values.hc1.data[2], isolevel) +
                    MC_ISOLEVEL_CHECK(values.hc1.data[3], isolevel) +
                    MC_ISOLEVEL_CHECK(values.hc1.data[4], isolevel) +
                    MC_ISOLEVEL_CHECK(values.hc1.data[5], isolevel) +
                    MC_ISOLEVEL_CHECK(values.hc1.data[6], isolevel) +
                    MC_ISOLEVEL_CHECK(values.hc1.data[7], isolevel) +
                    MC_ISOLEVEL_CHECK(values.hc1.data[8], isolevel));
            //
            for (x = lbd_x + 1; x < ubd_x; x++) {
                ...
                //
                pre1 = pre2;
                values.hc0 = values.hc1;
                //
                // read second halfcube.
                //
                values.hc1.c0 = grid[MC_COMPUTE_INDEX(dim_x, dim_xz, x, z, y)];
                values.hc1.c3 = grid[MC_COMPUTE_INDEX(dim_x, dim_xz, x, zz, y)];
                values.hc1.c6 =
                    grid[MC_COMPUTE_INDEX(dim_x, dim_xz, x, zzz, y)];
                values.hc1.c1 = grid[MC_COMPUTE_INDEX(dim_x, dim_xz, x, z, yy)];
                values.hc1.c4 =
    ...
    if ((dim_y % 2) == 0) {
        ...
        y = ubd_y;
        const int_type yy = y + 1;
        off_y1 = offset_y + (y * voxel_height);
        off_y2 = off_y1 + voxel_height;
        //
        off_z1 = offset_z + (lbd_z * voxel_depth);
        off_z2 = off_z1 + voxel_depth;
    ...
    if ((dim_z % 2) == 0) {
        ...
        z = ubd_z;
        const int_type zz = z + 1;
        off_z1 = offset_z + (z * voxel_depth);
        off_z2 = off_z1 + voxel_height;
        //
        // ignore possible last (y) row in next section, if y-slice
        // has been proceed.
        //
